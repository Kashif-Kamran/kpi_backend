...
let crash_errors = {
    "Caused by: java.lang.IllegalArgumentException: parameter must be a descendant of this view": ["Append a ScrollListener to your Activity, when listView start scrolling, clear current focus.",
        "Set prepare(mediaSource, true, true)"],
...
recomendationRouter.get("/",(req,res) =>
{
    res.send({
        status: true,
        message: "Recomendation Router is Working"
    });
});
// export this router
...
function main() {
    let error = prompt("Please enter the error: ");
    let errorType = getErrorType(error);

    if (errorType === "ANR") {
        if (anr_errors.hasOwnProperty(error)) {
            let solutions = anr_errors[error];
            for (let i = 0; i < solutions.length; i++) {
                let solution = solutions[i];
                console.log(`Solution for ${error}: ${solution}`);
                let response = prompt("Did this solution work for you? (y/n): ");
                if (response.toLowerCase() === "y") {
                    console.log("Thank you for using the program.");
                    return;
                }
            }
            console.log("All solutions failed. Please contact support.");
        } else {
            console.log("Error not found.");
        }
    } else if (errorType === "Crash") {
        if (crash_errors.hasOwnProperty(error)) {
            let solutions = crash_errors[error];
            for (let i = 0; i < solutions.length; i++) {
                let solution = solutions[i];
                console.log(`Solution for ${error}: ${solution}`);
                let response = prompt("Did this solution work for you? (y/n): ");
